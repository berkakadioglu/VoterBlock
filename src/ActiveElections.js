...
class ActiveElections extends Component {
  ...
  loadData = async () => {
    ...
    for (let i = 0; i < eCount; i++) {
      ...
      let voterData = await election.methods.Voters(this.app.account[0]).call();

      // Boolean indicating whether the contract address has voted or not
      electionDetails[i].hasVoted = voterData.isVoted;

      // Name of the election
      electionDetails[i].electionName = await election.methods.electionName().call();

      // Description of the election
      electionDetails[i].electionDescription = await election.methods.electionDescription().call();
      ...
      for (let j = 0; j < candidatesCount; j++) {
        ...
        candidateComponents[i].push(
          <Candidates
            name={candidates[i][j][0]}
            voteCount={candidates[i][j][1]}
          />
        );
      }
